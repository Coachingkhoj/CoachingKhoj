...
class Handler(SimpleHTTPRequestHandler):
    ...
    def json_with_session(self,code,obj,session):
        raw=json.dumps(obj).encode(); self.send_response(code); self.send_header('Content-Type','application/json; charset=utf-8'); self.send_header('X-Admin-Session',session); self.send_header('Content-Length',str(len(raw))); self.end_headers(); self.wfile.write(raw)
    def body(self):
        ...
        try:
            if path=='/api/admin/login':
                ...
                if secrets.compare_digest(str(x.get('password','')),ADMIN_PASSWORD):
                    t=secrets.token_urlsafe(32); SESSIONS[t]=time.time()+SESSION_TTL; return self.json_with_session(200,{'ok':True,'session':t},t)
